...
class SAM3UNetCNNDifussionv3(nn.Module):
    def __init__(self, checkpoint_path=None, img_size=336, num_classes=7,
        ...
        super(SAM3UNetCNNDifussionv3, self).__init__()
        ...
        # if vae_checkpoint:
        #     print(f"\n" + "="*60)
        #     print(f"正在读取 VAE 预训练文件: {vae_checkpoint}")
        #     vae_data = torch.load(vae_checkpoint, map_location="cpu")
            
        #     # 自动处理字典嵌套
        #     vae_state = vae_data["state_dict"] if isinstance(vae_data, dict) and "state_dict" in vae_data else vae_data

        #     # 这是一个万能匹配器，自动处理 module. 前缀并精准加载到子模块
        #     def robust_load(sub_module, prefix_name):
        #         # 1. 提取所有相关的键，并彻底清洗掉 'module.' 和 'mask_encoder.' 等前缀
        #         target_state = {}
        #         for k, v in vae_state.items():
        #             clean_k = k.replace("module.", "") # 处理多卡
        #             if clean_k.startswith(prefix_name + "."):
        #                 # 核心：只保留模块内部的路径，例如 net.0.weight
        #                 inner_k = clean_k[len(prefix_name) + 1:]
        #                 target_state[inner_k] = v
                
        #         if target_state:
        #             info = sub_module.load_state_dict(target_state, strict=False)
        #             print(f"-> [{prefix_name:<13}] 成功匹配 {len(target_state):>3} 个参数 | 缺失: {len(info.missing_keys)}")
        #         else:
        #             print(f"-> [{prefix_name:<13}] ❌ 警告：未在文件中找到匹配参数！")

        #     # 按前缀精准加载
        #     robust_load(self.image_encoder, "image_encoder")
        #     robust_load(self.mask_embed,    "mask_embed")
        #     robust_load(self.mask_encoder,  "mask_encoder")
        #     robust_load(self.mask_decoder,  "mask_decoder")
        #     print("="*60 + "\n")
        if vae_checkpoint:
    ...
    @torch.no_grad()
    def refine_sample(self, x, t_start: int = 200, num_steps: int = 10, T: int = 1000, betas=None, eta: float = 0.0):
        """
        ...
        xt = alpha_t.sqrt() * coarse_latent + (1.0 - alpha_t).sqrt() * torch.randn_like(coarse_latent)*0.1
