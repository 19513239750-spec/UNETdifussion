...
# ====================== 改进的评价函数 (支持迭代细化) ======================
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, num_classes: int, train_stage: str = "stage1") -> Dict[str, float]:
    model.eval()
    ...
    for imgs, masks in loader:
        ...
        if train_stage == "stage1":
            ...
            # 第二阶段必须使用精修采样逻辑
            # 注意：此处参数 t_start 和 num_steps 应与推理配置一致
            logits = model.refine_sample(imgs, t_start=200, num_steps=5)
...
def main():
    ...
    parser.add_argument("--train_stage", type=str, default="stage2", choices=["stage1", "stage2"])
    ...
    model = SAM3UNetCNNDifussionv3(checkpoint_path=args.sam3_ckpt, img_size=args.image_size, num_classes=7).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for imgs, masks in pbar:
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
                if args.train_stage == "stage1":
                    ...
                    coarse_logits, boundary_logits, noise_pred, repa_feat, visual_features = model(imgs, noisy_latent, t_float)
                    loss = stage2_criterion(coarse_logits, masks,
                                            boundary_logits=boundary_logits,
                                            noise_pred=noise_pred,
                                            true_noise=true_noise,
                                            repa_feat=repa_feat,
                                            teacher_feat=visual_features)
        ...
        # 评估与记录
        metrics = evaluate(model, val_loader, device, 7, train_stage=args.train_stage)
