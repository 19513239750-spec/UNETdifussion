...
# ====================== 1. 脚本硬编码配置 ======================
# 模型文件路径
MODEL_PY_PATH = "/workspace/sam3_Unet/sam3_unet/sam3/SAM3UNetCNNDifussionv2.py"
# 权重文件路径 (之前训练保存的 best.pt 或 final.pt)
CHECKPOINT_PATH = "/workspace/weights/SAM3UNetDiffusionv2/best.pt"
# 数据集根目录
...
SAM3UNetCNNDifussionv2 = _load_model_cls(MODEL_PY_PATH)
...
def run_test():
    ...
    # 2. 初始化模型并加载纯权重
    model = SAM3UNetCNNDifussionv2(img_size=IMAGE_SIZE, num_classes=NUM_CLASSES).to(device)
